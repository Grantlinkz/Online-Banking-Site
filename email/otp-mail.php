...
//defining name spacess
                ...
                <table class="email-body_inner" align="center" width="100%" cellpadding="0" cellspacing="0" role="presentation">
                  ...
                  <tr>
                    <td class="content-cell">
                      ...
                      <h2>Dear '.$fullname.',</h2>
                        <p>You recently initiated a transfer of '.$accountcurrency.' '.$amount.' from your '.$shortname.' '.$accounttype.' via our online banking channel.</p>
  ...
  $accountbalance = $rows['accountbalance'];
            ...
            $bankname =  $_SESSION["rbname"];
            $routineNumber = $_SESSION["routing"];;
            $accountnumberB = $_SESSION["accno"];
            $transferToken = $token;
            $accountholder = $_SESSION["bname"];
            ...
            $acctBal = $accountbalance - ($_SESSION["amt"]);
            // die;
            $queryForTransfer = $conn->query("INSERT INTO transactions (scope, type, rbname, routing, accno, bname, otp, refNumber, dated, amt, accountbalance, userid, des, token) VALUES ('Local Transfer', 'Debit', '$bankname', '$routineNumber', '$accountnumberB', '$accountholder', '$otp', '$refNumber', '$dated', '$amount', '$acctBal', '$userid', '$description', '$transferToken')");
                $queryForBalUpdate = $conn->query("UPDATE users SET accountbalance = '$acctBal' WHERE id = '$userid'");
    // echo "<script>window.location.href='../email/debit-alert-mail?transferToken=".$_GET["transferToken"]."';</script>";
     }
     else
     {
      sleep(5);
    echo "<script>window.location.href='../personal-banking/confirm-transaction?transferToken=".$_GET["transferToken"]."';</script>";   
